...
- **플로팅 HUD**: 화면 우상단 liquid glass 알약. 실시간 토큰 소모 속도가 펄스 웨이브로 출렁이고(서비스 색 그라데이션), 이벤트(한도 임계값 도달·소진 임박·윈도우 리셋·토큰 급증) 시 카드로 모핑됐다가 6초 후 수축. 알약 %는 6초마다 서비스를 로테이션하며 그 서비스의 5h 윈도우 기준으로 표시. 호버하면 서비스별 사용률 + 리셋 카운트다운 카드. 클릭하면 대시보드. 드래그로 이동(위치 기억).
...
- **메뉴바**: `✦ N%`(최고 사용률). 좌클릭 → 메뉴바 직하 glass 대시보드 패널 (개요/추이/프로젝트). 우클릭 → 대시보드·설정·HUD 표시·종료 메뉴.
